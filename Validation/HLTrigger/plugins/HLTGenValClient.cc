...
HLTGenValClient::HLTGenValClient(const ParameterSet& pset)
  ...
  for (vstring::const_iterator effProfileCmd = effProfileCmds.begin(); effProfileCmd != effProfileCmds.end();
    ...
    if (args.size() < 4) {
      LogInfo("HLTGenValClient") << "Wrong input to effProfileCmds\n";
      continue;
    }
  ...
  for (vstring::const_iterator resCmd = resCmds.begin(); resCmd != resCmds.end(); ++resCmd) {
    ...
    if (args.size() != 3) {
      LogInfo("HLTGenValClient") << "Wrong input to resCmds\n";
      continue;
    }
  ...
  for (const auto& profileCmd : profileCmds) {
    ...
    if (args.size() != 3) {
      LogInfo("HLTGenValClient") << "Wrong input to profileCmds\n";
      continue;
    }
  ...
  for (vstring::const_iterator normCmd = normCmds.begin(); normCmd != normCmds.end(); ++normCmd) {
    ...
    if (args.empty() or args.size() > 2) {
      LogInfo("HLTGenValClient") << "Wrong input to normCmds\n";
      continue;
    }
  ...
  for (vstring::const_iterator cdCmd = cdCmds.begin(); cdCmd != cdCmds.end(); ++cdCmd) {
    ...
    if (args.empty() || args.size() > 2) {
      LogInfo("HLTGenValClient") << "Wrong input to cdCmds\n";
      continue;
    }
  ...
  for (vstring::const_iterator noFlowCmd = noFlowCmds.begin(); noFlowCmd != noFlowCmds.end(); ++noFlowCmd) {
    ...
    if (args.empty() || args.size() > 2) {
      LogInfo("HLTGenValClient") << "Wrong input to noFlowCmds\n";
      continue;
    }
...
void HLTGenValClient::computeResolution(DQMStore::IBooker& ibooker,
                                         DQMStore::IGetter& igetter,
                                         const string& startDir,
  ...
  if (!igetter.dirExists(startDir)) {
    if (verbose_ >= 2 || (verbose_ == 1 && !isWildcardUsed_)) {
      LogInfo("HLTGenValClient") << "computeResolution() : "
                                  << "Cannot find sub-directory " << startDir << endl;
    }
    return;
  }
...
void HLTGenValClient::computeProfile(DQMStore::IBooker& ibooker,
                                      DQMStore::IGetter& igetter,
                                      const std::string& startDir,
                                      const std::string& profileMEName,
                                      const std::string& profileMETitle,
  ...
  if (!igetter.dirExists(startDir)) {
    if (verbose_ >= 2 || (verbose_ == 1 && !isWildcardUsed_)) {
      LogInfo("HLTGenValClient") << "computeProfile() : "
                                  << "Cannot find sub-directory " << startDir << endl;
    }
    return;
  }
...
void HLTGenValClient::normalizeToEntries(DQMStore::IBooker& ibooker,
                                          DQMStore::IGetter& igetter,
                                          const std::string& startDir,
                                          const std::string& histName,
                                          const std::string& normHistName) {
  if (!igetter.dirExists(startDir)) {
    if (verbose_ >= 2 || (verbose_ == 1 && !isWildcardUsed_)) {
      LogInfo("HLTGenValClient") << "normalizeToEntries() : "
                                  << "Cannot find sub-directory " << startDir << endl;
    }
    return;
  }
...
void HLTGenValClient::makeCumulativeDist(DQMStore::IBooker& ibooker,
                                          DQMStore::IGetter& igetter,
                                          const std::string& startDir,
                                          const std::string& cdName,
                                          bool ascending) {
  if (!igetter.dirExists(startDir)) {
    if (verbose_ >= 2 || (verbose_ == 1 && !isWildcardUsed_)) {
      LogInfo("HLTGenValClient") << "makeCumulativeDist() : "
                                  << "Cannot find sub-directory " << startDir << endl;
    }
    return;
  }
...
void HLTGenValClient::makeNoFlowDist(DQMStore::IBooker& ibooker,
                                      DQMStore::IGetter& igetter,
                                      const std::string& startDir,
                                      const std::string& noFlowName) {
  if (!igetter.dirExists(startDir)) {
    if (verbose_ >= 2 || (verbose_ == 1 && !isWildcardUsed_)) {
      LogInfo("HLTGenValClient") << "makeNoFlowDist() : "
                                  << "Cannot find sub-directory " << startDir << endl;
    }
    return;
  }
...
void HLTGenValClient::limitedFit(MonitorElement* srcME, MonitorElement* meanME, MonitorElement* sigmaME) {
  ...
  for (int i = 1; i <= binx; i++) {
    ...
    if (cont >= cont_min) {
      ...
      //To avoid HLTGenValClient maintains state between fit calls
